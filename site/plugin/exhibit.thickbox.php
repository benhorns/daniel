...
function createExhibit()
{
	...
	foreach ($pages as $go)
	{
		$a .= "\n<a href='" . BASEURL . GIMGS . "/$go[media_file]' title='$go[media_title]' class='thickbox' rel='gallery-plants'><img src='" . BASEURL . GIMGS . "/th-$go[media_file]' alt='$go[media_caption]' height='' /></a>\n";
		
		$i++;
	}
	
	// images
	$s .= "<div id='img-thickbox'>\n";
	$s .= $a;
	$s .= "</div>\n";
...
function dynamicCSS()
{
	return "a.thickbox { border: none; }\na.thickbox img { border: 3px solid #fff; margin: 0 6px 9px 0; font-style: italic; }
	#a.thumb-img #title { font-style: italic; text-align: right;}";
